// app.jsx - Definitive Working Version with All Features
...
// --- Reusable UI Components (Defined first, globally) ---
...
const SearchBar = ({ onSearch, onResultClick, searchResults }) => {
    ...
    const debouncedQuery = useDebounce(query, 300); const searchRef = useRef(null); const [isFocused, setIsFocused] = useState(false);
    useEffect(() => { onSearch(debouncedQuery); }, [debouncedQuery, onSearch]);
    useEffect(() => { const handleClickOutside = (event) => { if (searchRef.current && !searchRef.current.contains(event.target)) setIsFocused(false); }; document.addEventListener("mousedown", handleClickOutside); return () => document.removeEventListener("mousedown", handleClickOutside); }, []);
    const handleResultItemClick = (result) => { setQuery(''); setIsFocused(false); onResultClick(result); };
    return ( <div ref={searchRef} className="search-container"> <div className="search-icon"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg></div> <input type="text" placeholder="Search for a movie or TV show..." value={query} onChange={(e) => setQuery(e.target.value)} onFocus={() => setIsFocused(true)} className="search-input" /> {isFocused && searchResults.length > 0 && ( <div className="search-results-dropdown">{searchResults.map(result => ( <div key={result.id} className="search-result-item" onClick={() => handleResultItemClick(result)}><img src={result.poster} alt={result.title} /><div className="search-result-info"><span className="search-result-title">{result.title}</span><span className="search-result-year">{result.year}</span></div></div>))}</div>)}</div>);
};

const AdvancedFilters = ({ genres, platforms, filters, onFilterChange }) => {
    const { useState, useRef } = React;
    const [isOpen, setIsOpen] = useState(false); const [platformSearch, setPlatformSearch] = useState(''); const contentRef = useRef(null);
    const handleGenreChange = (genreId, type) => { const currentList = filters[type]; const newList = currentList.includes(genreId) ? currentList.filter(id => id !== genreId) : [...currentList, genreId]; const otherType = type === 'includeGenres' ? 'excludeGenres' : 'includeGenres'; const otherList = filters[otherType].filter(id => id !== genreId); onFilterChange({ ...filters, [type]: newList, [otherType]: otherList }); };
    const handlePlatformChange = (platformId) => { const newList = filters.platforms.includes(platformId) ? filters.platforms.filter(id => id !== platformId) : [...filters.platforms, platformId]; onFilterChange({ ...filters, platforms: newList }); };
...
const App = () => {
    ...
    useEffect(() => {
        ...
        const r = document.documentElement; r.style.setProperty('--color-accent', accent.color); r.style.setProperty('--color-accent-text', accent.text); r.style.setProperty('--color-accent-gradient-from', accent.from); r.style.setProperty('--color-accent-gradient-to', accent.to);
        ...
        const rgb = hexToRgb(accent.color); if (rgb) r.style.setProperty('--color-accent-rgb', `${rgb.r},${rgb.g},${rgb.b}`);
        localStorage.setItem('moviePicker_accent', JSON.stringify(accent));
    }, [displayMode, accent]);
    
    useEffect(() => { if (userRegion) localStorage.setItem('moviePicker_userRegion', userRegion); else localStorage.removeItem('moviePicker_userRegion'); localStorage.setItem('moviePicker_filters_v2', JSON.stringify(filters)); }, [userRegion, filters]);

    const fetchApi = useCallback(async (path, params = {}) => {
        if (!TMDB_API_KEY || TMDB_API_KEY === 'YOUR_API_KEY_HERE') throw new Error("API Key is missing!");
        const query = new URLSearchParams({ api_key: TMDB_API_KEY, ...params });
        const response = await fetch(`${TMDB_BASE_URL}/${path}?${query}`);
        if (!response.ok) { const data = await response.json(); throw new Error(data.status_message); }
        return response.json();
    }, []);

    const processMediaDetails = useCallback((details, mediaType, region) => {
        const trailer = details.videos?.results?.find(v => v.site === 'YouTube' && (v.type === 'Trailer' || v.type === 'Teaser'));
        return {
            title: details.title || details.name, id: details.id, year: (details.release_date || details.first_air_date || 'N/A').substring(0, 4), poster: details.poster_path ? `${TMDB_IMAGE_BASE_URL}${details.poster_path}` : 'https://via.placeholder.com/500x750.png?text=No+Image', rating: details.vote_average?.toFixed(1) || 'N/A', overview: details.overview,
            providers: details['watch/providers']?.results?.[region]?.flatrate || [], cast: details.credits?.cast?.slice(0, 10) || [],
            similar: details.similar?.results?.filter(s => s.poster_path).slice(0, 10).map(s => ({...s, mediaType}) ) || [],
            trailerKey: trailer ? trailer.key : null, mediaType: mediaType,
        };
    }, []);
    ...
    useEffect(() => {
        ...
        fetchApi('configuration/countries').then(data => {
            setAvailableRegions(data.filter(c => TOP_COUNTRIES.includes(c.iso_3166_1)).sort((a,b) => a.english_name.localeCompare(b.english_name)));
            setAppStatus('ready');
    ...
    useEffect(() => {
        ...
        const fetchRegionData = async () => { try { const [genresData, platformsData] = await Promise.all([fetchApi(`genre/${mediaType}/list`), fetchApi(`watch/providers/${mediaType}`, { watch_region: userRegion })]); setAllGenres(genresData.genres || []); setAllPlatforms(platformsData.results || []); } catch (err) { setError("Could not load data for the selected region."); }};
        fetchRegionData();
    ...
    const handleSurpriseMe = useCallback(async () => {
        ...
        try {
            ...
            const results = pageData.results.filter(r => r.poster_path);
    ...
    const handleSearch = useCallback(async (query) => {
        ...
        try { const data = await fetchApi('search/multi', { query }); const filtered = data.results.filter(r => (r.media_type === 'movie' || r.media_type === 'tv') && r.poster_path).slice(0, 5).map(r => ({ id: r.id, title: r.title || r.name, year: (r.release_date || r.first_air_date || '').substring(0, 4), poster: `${TMDB_IMAGE_BASE_URL}${r.poster_path}`, mediaType: r.media_type, })); setSearchResults(filtered); } catch (err) { console.error("Search failed:", err); }
    }, [fetchApi]);

    const handleResultClick = useCallback(async (result) => {
        setError(null); setSelectedMedia(null); setIsDiscovering(true);
        try { const details = await fetchApi(`${result.mediaType}/${result.id}`, { append_to_response: 'videos,credits,similar,watch/providers' }); setSelectedMedia(processMediaDetails(details, result.mediaType, userRegion)); } catch (err) { setError("Could not fetch details."); } finally { setIsDiscovering(false); }
    }, [fetchApi, userRegion, processMediaDetails]);

    const handleActorClick = useCallback(async (actorId) => {
        try { setModal({ type: 'actor', data: null }); const actorDetails = await fetchApi(`person/${actorId}`, { append_to_response: 'movie_credits,tv_credits' }); setModal({ type: 'actor', data: actorDetails }); } catch (err) { setModal({ type: null, data: null }); }
    }, [fetchApi]);
