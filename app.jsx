// app.jsx - Definitive Final Version with All Fixes and Readability Improvements
...
const getSeasonalGenres = () => {
    const month = new Date().getMonth(); // 0=Jan, 9=Oct, 11=Dec
...
const ErrorDisplay = ({ message }) => (
    <div className="min-h-screen flex items-center justify-center p-4">
        <div className="w-full max-w-md text-center bg-red-900/50 border border-red-700 p-6 rounded-lg">
            ...
            <p className="text-red-200">{message || "An unknown error occurred."}</p>
        </div>
    </div>
);
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="relative w-full max-w-4xl" onClick={e => e.stopPropagation()}>
                ...
                <div className="trailer-responsive"><iframe src={`https://www.youtube.com/embed/${trailerKey}?autoplay=1&rel=0`} frameBorder="0" allow="autoplay; encrypted-media" allowFullScreen title="Movie Trailer"></iframe></div>
            </div>
        </div>
    );
};
...
const ActorDetailsModal = ({ actor, onClose }) => {
    if (!actor) return <div className="modal-overlay"><Loader /></div>;
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content relative w-full max-w-4xl" onClick={e => e.stopPropagation()}>
                <button onClick={onClose} className="modal-close-btn text-white"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
                <div className="overflow-y-auto">
                    <div className="p-6 md:flex md:space-x-8">
                        <div className="md:w-1/3 flex-shrink-0 text-center"><img src={actor.profile_path ? `${TMDB_IMAGE_BASE_URL}${actor.profile_path}` : 'https://via.placeholder.com/500x750.png?text=No+Image'} alt={actor.name} className="rounded-lg shadow-lg w-2/3 md:w-full mx-auto" /></div>
                        <div className="mt-4 md:mt-0 md:w-2/3"><h2 className="text-3xl md:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[var(--color-accent-gradient-from)] to-[var(--color-accent-gradient-to)]">{actor.name}</h2><p className="mt-4 text-base leading-relaxed max-h-60 overflow-y-auto">{actor.biography || "No biography available."}</p></div>
                    </div>
                    {knownFor.length > 0 && (<div className="p-6 border-t border-[var(--color-card-border)]"><h3 className="text-xl font-semibold mb-4 text-[var(--color-accent-text)]">Known For</h3><div className="horizontal-scroll-container">{knownFor.map(item => (<div key={item.credit_id || item.id} className="flex-shrink-0 w-32 text-center"><img src={`${TMDB_IMAGE_BASE_URL}${item.poster_path}`} alt={item.title || item.name} className="rounded-lg" /><span className="block mt-2 text-xs text-gray-400">{item.title || item.name}</span></div>))}</div></div>)}
                </div>
            </div>
        </div>
    );
...
const SettingsMenu = ({ onCountryChange, onSelectTheme, currentTheme, displayMode, onDisplayModeChange, onOpenWatched, onOpenWatchlist, t }) => {
...
const MovieCardModal = ({ media, onClose, onToggleWatched, isWatched, onToggleWatchlist, isWatchlisted, onActorClick, onSimilarClick }) => { if (!media) return null; return (<div className="modal-overlay" onClick={onClose}><div className="movie-card-modal-content" onClick={e=>e.stopPropagation()}><button onClick={onClose} className="modal-close-btn text-white"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button><div className="movie-card-modal-body"><div className="p-6 sm:p-8 sm:flex sm:space-x-8"><div className="sm:w-1/3 flex-shrink-0"><img src={media.poster} alt={`Poster for ${media.title}`} className="rounded-lg shadow-lg w-full" />{media.trailerKey && (<button onClick={()=>onClose({type:'trailer',data:media.trailerKey})} className="card-action-btn trailer-btn mt-4">Watch Trailer</button>)}</div><div className="mt-6 sm:mt-0 flex-grow text-left"><h2 className="text-3xl lg:text-4xl font-bold">{media.title}</h2><div className="flex items-center flex-wrap gap-x-4 gap-y-1 text-[var(--color-text-secondary)] mt-2"><span>{media.year}</span>{media.certification && <span className="border border-[var(--color-card-border)] px-2 py-0.5 rounded text-xs">{media.certification}</span>}{media.duration > 0 && <span>{Math.floor(media.duration/60)}h {media.duration % 60}m</span>}{media.seasons && <span>{media.seasons} Season(s)</span>}<span>⭐ {media.rating}</span></div><p className="mt-4 text-base leading-relaxed">{media.overview}</p>{media.director && <p className="mt-4"><span className="font-semibold text-gray-400">Director:</span> {media.director}</p>}<div className="mt-6"><h3 className="font-semibold text-lg">Available on (Subscription)</h3>{media.providers.length>0?(<div className="flex flex-wrap gap-2 mt-2">{media.providers.map(p => <img key={p.provider_id} src={`${TMDB_IMAGE_BASE_URL}${p.logo_path}`} alt={p.provider_name} title={p.provider_name} className="platform-logo" />)}</div>):<p className="text-[var(--color-text-secondary)]">Not found on streaming services.</p>}</div>{media.rentProviders.length>0 && (<div className="mt-4"><h3 className="font-semibold text-lg">Available for Rent/Buy</h3><div className="flex flex-wrap gap-2 mt-2">{media.rentProviders.map(p=><img key={p.provider_id} src={`${TMDB_IMAGE_BASE_URL}${p.logo_path}`} alt={p.provider_name} title={p.provider_name} className="platform-logo"/>)}</div></div>)}<div className="card-user-actions mt-6 pt-6 border-t border-[var(--color-card-border)]"><button onClick={() => onToggleWatched(media)} className={`user-action-btn watch-btn ${isWatched ? 'active':''}`}> <svg fill="currentColor" viewBox="0 0 20 20"><path d="M10 12a2 2 0 100-4 2 2 0 000 4z" /><path fillRule="evenodd" d="M.458 10C3.732 4.943 9.522 3 10 3s6.268 1.943 9.542 7c-3.274 5.057-9.064 7-9.542 7S3.732 15.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clipRule="evenodd" /></svg> {isWatched ? 'Watched':'Mark as Watched'} </button><button onClick={() => onToggleWatchlist(media)} className={`user-action-btn watchlist-btn ${isWatchlisted ? 'active':''}`}> <svg fill="currentColor" viewBox="0 0 20 20"><path d="M5 4a2 2 0 012-2h6a2 2 0 012 2v14l-5-3.13L5 18V4z" /></svg> {isWatchlisted ? 'Saved':'Save for Later'} </button></div></div></div>{media.cast.length>0&&(<div className="mt-8 pt-6 p-6 sm:p-8 border-t border-[var(--color-card-border)]"><h3 className="text-xl font-semibold mb-4 text-left text-[var(--color-accent-text)]">Main Cast</h3><div className="horizontal-scroll-container">{media.cast.map(actor=>(<button key={actor.id} className="flex-shrink-0 w-24 text-center actor-card" onClick={()=>onActorClick(actor.id)}><img src={actor.profile_path?`${TMDB_IMAGE_BASE_URL}${actor.profile_path}`:'https://via.placeholder.com/200x300.png?text=?'} alt={actor.name} className="actor-thumbnail" /><span className="actor-name">{actor.name}</span></button>))}</div></div>)}{media.similar.length > 0 && (<div className="mt-8 pt-6 p-6 sm:p-8 border-t border-[var(--color-card-border)]"><h3 className="text-xl font-semibold mb-4 text-left text-[var(--color-accent-text)]">Similar Titles</h3><div className="horizontal-scroll-container">{media.similar.map(item=>(<button key={item.id} className="flex-shrink-0 w-32 text-center actor-card" onClick={()=>onSimilarClick(item)}><img src={`${TMDB_IMAGE_BASE_URL}${item.poster_path}`} alt={item.title||item.name} className="rounded-lg w-full" /><span className="block mt-2 text-xs text-gray-400">{item.title||item.name}</span></button>))}</div></div>)}</div></div></div>);};
...
const App = () => {
    ...
    const [filters, setFilters] = useLocalStorage('whattowatch_filters_v5', { quickGenre: '', includeGenres: [], excludeGenres: [], platforms: [], indie: false });
    ...
    const showMediaAsModal = (mediaData) => setModal({ type: 'movieCard', data: mediaData });

    const handleSurpriseMe = useCallback(async () => {
        setIsDiscovering(true); setError(null);
        try { const params={'sort_by':'popularity.desc','watch_region':userRegion,'with_watch_monetization_types':'flatrate,rent,buy','vote_count.gte':filters.indie?10:300}; const genresToInclude=[filters.quickGenre,...filters.includeGenres].filter(Boolean).join(','); if (genresToInclude) params['with_genres']=genresToInclude; if(filters.excludeGenres.length)params['without_genres']=filters.excludeGenres.join(','); if(filters.platforms.length)params['with_watch_providers']=filters.platforms.join('|'); const data=await fetchApi(`discover/${mediaType}`,params); const totalPages=Math.min(data.total_pages,500); if(totalPages===0)throw new Error(t.noResults); const pageData=await fetchApi(`discover/${mediaType}`,{...params,page:Math.floor(Math.random()*totalPages)+1}); const results=pageData.results.filter(r=>r.poster_path&&!watchedList[r.id]); if(results.length===0) { throw new Error("No new results with these filters. Try changing them!"); } const randomResult=results[Math.floor(Math.random()*results.length)]; const details=await fetchApi(`${mediaType}/${randomResult.id}`,{append_to_response:'videos,credits,similar,watch/providers,release_dates'}); showMediaAsModal(processMediaDetails(details,mediaType,userRegion));
    ...
    return (
        <div className="container mx-auto max-w-4xl p-4 sm:p-6">
            <header className="app-header"><div className="app-title-container"><h1 className="app-title">{t.title}</h1><p className="app-subtitle">{t.subtitle}</p></div><div className="header-actions"><SearchBar onSearch={handleSearch} searchResults={searchResults.map(r=>({...r, poster: `${TMDB_IMAGE_BASE_URL}${r.poster}`}))} onResultClick={handleResultClick} /><SettingsMenu t={t} currentTheme={accent} onSelectTheme={setAccent} onCountryChange={handleChangeCountry} displayMode={displayMode} onDisplayModeChange={setDisplayMode} onOpenWatched={() => setModal({type:'watched'})} onOpenWatchlist={() => setModal({type:'watchlist'})}/></div></header>
            <main className="text-center">
                <div className="flex flex-col md:flex-row justify-center items-center gap-4 my-6"><div className="inline-flex p-1 rounded-full media-type-switcher"><button onClick={() => setMediaType('movie')} className={`px-5 py-2 text-sm rounded-full media-type-btn ${mediaType==='movie'?'media-type-btn-active':''}`}>{t.movies}</button><button onClick={() => setMediaType('tv')} className={`px-5 py-2 text-sm rounded-full media-type-btn ${mediaType==='tv'?'media-type-btn-active':''}`}>{t.tvShows}</button></div><button onClick={() => setIsFilterModalOpen(true)} className="filter-btn"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path></svg>Filters</button><button onClick={handleSurpriseMe} disabled={isDiscovering} className="w-full md:w-auto px-8 py-3 text-lg font-bold text-white rounded-full surprise-me-btn disabled:opacity-50 disabled:cursor-not-allowed">{isDiscovering ? 'Searching...' : 'Surprise Me!'}</button></div>
                {isDiscovering && <div className="loader mx-auto mt-8"></div>}
                {error && <p className="text-red-400 mt-4">{error}</p>}
                {!isDiscovering && !modal.type && <p className="text-gray-400 max-w-lg mx-auto">Use the search bar to find a specific title, or use the filters and hit "Surprise Me!" for a random suggestion.</p>}
            </main>
            {modal.type === 'movieCard' && <MovieCardModal media={modal.data} onClose={(newModal={type:null}) => setModal(newModal)} isWatched={!!watchedList[modal.data?.id]} isWatchlisted={!!watchList[modal.data?.id]} onToggleWatched={handleToggleWatched} onToggleWatchlist={handleToggleWatchlist} onActorClick={(id)=>{setModal({type:null});setTimeout(()=>handleActorClick(id),200);}} onSimilarClick={(item)=>{setModal({type:null});setTimeout(()=>handleResultClick(item),200);}}/>}
